  ...
  return (
    <div className="min-h-screen bg-[#F5F5F7]" data-testid="report-page">
      ...
      <main className="max-w-7xl mx-auto px-6 md:px-8 py-10 md:py-14">
        ...
        {/* ISSUES */}
        <div className="mt-6 card-elev p-6 md:p-8" data-testid="report-issues">
          <div className="flex items-center justify-between mb-5">
            <div className="text-xs uppercase tracking-[0.2em] text-[#86868B]">Issues</div>
            <div className="text-sm text-[#86868B]">{(s.issues || []).length} total</div>
          </div>
          <div className="space-y-3">
            {(s.issues || []).map((i) => (
              <div key={i.id} className="flex items-start gap-4 rounded-xl border border-black/5 p-4" data-testid={`issue-row-${i.id}`}>
                <span className="mt-1.5 w-1.5 h-1.5 rounded-full shrink-0" style={{ background: SEV_COLOR[i.severity] || "#86868B" }} />
                <div className="flex-1">
                  <div className="flex items-center gap-3 flex-wrap">
                    <span className="text-[10px] uppercase tracking-[0.2em] text-[#86868B]">{i.category}</span>
                    <span className="text-[10px] uppercase tracking-[0.2em]" style={{ color: SEV_COLOR[i.severity] || "#86868B" }}>{i.severity}</span>
                  </div>
                  <div className="font-display text-lg font-medium mt-1">{i.title}</div>
                  <div className="font-mono text-xs text-[#86868B] mt-1">{i.file}</div>
                  <div className="text-sm text-[#1D1D1F]/70 mt-2">Likely cause: {i.cause}</div>
                </div>
              </div>
            ))}
          </div>
